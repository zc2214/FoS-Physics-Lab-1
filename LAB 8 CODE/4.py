...
x, y = np.loadtxt("4.txt", skiprows=1 , unpack=True)
def LineFit(x, y):
    xavg = x.mean()
...
plt.grid(True)
plt.text(0.1, 19.5, r'$y=A+Bx$', fontsize=12)
plt.text(0.1, 18.5, r'A:$%s\pm%s$'% (yint, d_yint), fontsize=12)
plt.text(0.1, 17.5, r'B:$%s\pm%s$'% (slope, d_slope), fontsize=12)
plt.title('Angular Velocity vs Time 4\n')

plt.savefig("Angular_Velocity_and_Time 4.png")
plt.show()
